class UserAnswer < ActiveRecord::Base
  attr_accessible :content, :question, :survey_result, :question_id
  belongs_to :question
  ...
  validates :content, :presence => { :unless => :question_has_answers? }, :format => { :with => /^[0-9]+$/, :if => :answer_type_number?, :message => 'must be a number!' }
